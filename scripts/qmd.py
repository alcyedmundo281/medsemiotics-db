...
def entrada_concepto_md(cid: str, concepto: dict) -> list:
    ...
    if isinstance(umbral, dict) and umbral.get("parametro"):
        ...
        detalle.append(md_texto(f"umbral: {umbral['parametro']} {' / '.join(cortes)}"))
    return [
...
def capitulo_vocabulario(indice: banco.Indice) -> str:
    lineas = ["# Vocabulario", ""]
    ubicados: set = set()
    ...
    for raiz_id, titulo in banco.GRUPOS_VOCABULARIO:
        ...
        for cid in hijos:
            ubicados.add(cid)
            lineas += entrada_concepto_md(cid, indice.concepto(cid))
    ...
    if huerfanos:
        ...
        for cid in huerfanos:
            lineas += entrada_concepto_md(cid, indice.conceptos[cid])
...
def figura_markdown(medio: dict) -> str:
    ...
    """
    pie = (
        f"{md_texto(medio['descripcion'])}. {md_texto(medio['credito'])}. "
...
def figuras_de_condicion(
    ...
    """Una figura por cada concepto ilustrado entre los signos de la condición,
    en el orden en que aparecen y sin repetir el mismo concepto dos veces."""
    lineas: list = []
    vistos: set = set()
    for arista in signos:
        ...
        medio = banco.imagen_de_concepto(
            indice, concepto, f"{donde} ({concepto.get('termino')})"
        )
        if medio:
            vistos.add(cid)
            recogidas.append((concepto, medio))
...
def bloque_escalas(condicion: dict) -> list:
    ...
    for escala in escalas:
        ...
        for tramo in escala.get("tramos") or []:
            ...
            if tramo.get("lr_positivo"):
                cocientes.append(f"LR+ {banco.numero(tramo['lr_positivo'])}")
            if tramo.get("lr_negativo"):
                cocientes.append(f"LR- {banco.numero(tramo['lr_negativo'])}")
            lineas.append(
                f"- **{md_texto(tramo.get('rango', ''))}**: {md_texto(', '.join(cocientes))}"
            )
        lineas.append("")
        if escala.get("decision"):
            lineas += [md_texto(frase(escala["decision"])), ""]
    return lineas
...
def capitulo_condicion(
    ...
    lineas += bloque_escalas(condicion)
...
def generar(indice: banco.Indice, raiz: Path, destino: Path) -> dict:
    ...
    textos = {"vocabulario.qmd": capitulo_vocabulario(indice)}
