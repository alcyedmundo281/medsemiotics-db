...
def imagen_de_concepto(indice: Indice, concepto: dict, donde: str):
    """La primera imagen válida de un concepto, o aborta si trae `medios` con
    metadatos incompletos. build.py ya lo valida; esto repite el mínimo que el
    libro necesita para no tipografiar una figura sin atribución."""
...
def filas_de_signo(indice: Indice, arista: dict, donde: str):
    ...
    if arista.get("odds_ratio") is not None:
        ...
        if isinstance(bruto, dict):
            sobrantes_or = sorted(set(bruto) - CLAVES_LR_CONOCIDAS)
            if sobrantes_or:
                raise ErrorGeneracion(
